import { 
	...
	ANALYSE_EMAILS, COLLECT_PROMPT_DATA, SEND_PROMPT_DATA_SDK
...
(function () {
	var s = {
		...
		addChromeListeners: function () {
			...
			BROWSER.runtime.onMessage.addListener(function (request, sender, sendResponse) {
				if (typeof request.id != "undefined") {
					...
					} else if (request.id == SEND_PROMPT_DATA_SDK) {
						_this.sendPromptDataToServer(request.data)
					}
				}
				return true;
			});
			...
			BROWSER.runtime.onInstalled.addListener((details) => {
				if (details.reason === "install") BROWSER.tabs.create({ url: DOMAIN_NAME + 'how-it-works' });
			});
		...
		setBadgeNumber: async function() {
			...
			if(tabInfo?.response) {
				const responseKey = Object.keys(tabInfo.response)[0];
				const techArray = tabInfo.response[responseKey];
				
				const techName = Object.values(techArray)
			...
			if(count > 0) {
				...
				chrome.action.setBadgeBackgroundColor({ color: '#00D96C' });
			} else {
		...
		sendPromptDataToServer: async function name(data) {
			try {
				await _this.makeRequest({
					type: "POST",
					url: COLLECT_PROMPT_DATA,
					reqBody: data
				});
			} catch (e) {
				console.log(e)
							...
							if (isMatch && pageDoms.length > 0) {
								...
								BROWSER.tabs.sendMessage( tabId, {
									...
									data: { 
										...
										doms: pageDoms, 
										rawData: tabResp,
										isPromptDom: tabResp?.dom_data?.is_prompt_dom ? true : false,
										promptVendor: tabResp?.dom_data?.prompt_vendor || ""
									}
								}, function (response) { });
							}
						}
    ...
    makeRequest: async function(params) {
      ...
      return response;
		...
		getSiteAppsFromServer: function (tabId, rawhostname, href, data, cb) {
			// verify domain
			if (!_this.isValidDomain(rawhostname) || rawhostname === undefined) return false;
				...
				try {
					fetch(GET_SITE_APPS, {
						...
						try {
						  ...
						  _this.getSiteAppsCallBack(tabId, responseText, rawhostname, hostname, href, data, cb);
						} catch (e) {
						  console.log(e);
						}
					}).catch(error => {
		...
		getSiteAppsCallBack: function (tabId, response, rawhostname, hostname, href, data, cb) {
			try {
				let currentResponse = {
					response: response,
					data: data
		...
		analyseSiteApps: function (tabId, rawhostname, hostname, href, data, callback, retry = false) {
			try {
				if (tabResponseData[tabId] != null) {
					...
					if (!tabResponseData[tabId]) {
						_this.getSiteAppsFromServer(tabId, rawhostname, href, data, () => {
		...
		processData: function (tabId, response, rawhostname, hostname, href, data) {
			try {
				...
				if (response?.update_data === true || !response.status) {
