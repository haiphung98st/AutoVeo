	...
	var su = {
		...
		processHTML: function (html) {
			try {
				...
				html += scripts;
				html += this.shopifyThemeScripts();
			} catch (e) {
				console.log(e);
			}
			return html;
		},
		shopifyThemeScripts: function () {
			// Get all inline script contents from the page
			const scriptContents = Array.from(document.getElementsByTagName("script"))
				.map(script => script.innerHTML);

			// Filter scripts that contain 'Shopify.theme'
			const shopifyScripts = scriptContents.filter(content => content.includes("Shopify.theme"));

			inlineScripts = ''
			shopifyScripts.map(x=> inlineScripts += `<script>${x}</script>`)
